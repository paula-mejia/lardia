...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!employer) {
      const { data: newEmployer, error: insertErr } = await supabase
        .from('employers')
        .insert({
          user_id: user.id,
          email: user.email,
      ...
      if (insertErr) {
        ...
        return NextResponse.json({ error: `Erro ao criar perfil: ${insertErr.message}` }, { status: 500 })
      }
    ...
    // Price ID from server-side env
    const priceId = process.env.STRIPE_PRICE_ID_BACKGROUND_CHECK
      || process.env.NEXT_PUBLIC_STRIPE_PRICE_ID_BACKGROUND_CHECK

    if (!priceId) {
      return NextResponse.json({ error: 'Price ID not configured. Set STRIPE_PRICE_ID_BACKGROUND_CHECK env var.' }, { status: 500 })
    }
    ...
    console.error('Background check checkout error:', err)
