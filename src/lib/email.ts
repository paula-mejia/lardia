  ...
  return resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #059669;">Bem-vindo ao LarDia!</h1>
        <p>Olá${name ? ` ${name}` : ''},</p>
        ...
        <p><a href="https://lardia.com.br/dashboard" style="background: #059669; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Acessar meu painel</a></p>
...
export async function sendDaeReminderEmail(to: string, name: string, month: string, dueDate: string, totalValue: string) {
  return resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #059669;">Lembrete de pagamento DAE</h1>
        <p>Olá${name ? ` ${name}` : ''},</p>
        ...
        <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 16px; margin: 16px 0;">
          ...
          <p style="margin: 4px 0 0; font-size: 24px; font-weight: bold; color: #059669;">${totalValue}</p>
        </div>
        <p><a href="https://lardia.com.br/dashboard/esocial/dae" style="background: #059669; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Ver DAE e gerar boleto</a></p>
...
export async function sendPayrollProcessedEmail(to: string, name: string, month: string, employeeCount: number, totalValue: string) {
  return resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #059669;">Folha processada ✓</h1>
        <p>Olá${name ? ` ${name}` : ''},</p>
        ...
        <p><a href="https://lardia.com.br/dashboard/esocial" style="background: #059669; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Ver detalhes</a></p>
        <hr style="border: none; border-top: 1px solid #eee; margin: 24px 0;" />
        <p style="color: #999; font-size: 12px;">LarDia - Seu lar em dia | lardia.com.br</p>
      </div>
    `,
